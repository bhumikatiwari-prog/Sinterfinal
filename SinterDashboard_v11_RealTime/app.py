...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #1a3a5c 0%, #e8f0f7 100%);
...
.js-plotly-plot .plotly .bg { fill: #f8fafc !important; }
...
[data-testid="stMetricValue"] { color:#1a3a5c !important; }
[data-testid="stMetricLabel"] { color:#6b7a8d !important; }
[data-testid="stMetricDelta"] { font-size:0.8rem !important; }
 
/* Tab styling */
.stTabs [data-baseweb="tab-list"] {
    background: #ffffff; border-bottom:1px solid #d1dce8; gap:4px;
}
...
.stTabs [aria-selected="true"] {
    background: #f8fafc !important; color:#1a3a5c !important;
    border-bottom: 2px solid #2563eb !important;
}
div[data-baseweb="tab-panel"] { background:#ffffff; padding-top:16px; }
...
CHART_LAYOUT = dict(
    ...
    xaxis=dict(gridcolor='#d1dce8',showgrid=True,zeroline=False),
    yaxis=dict(gridcolor='#d1dce8',showgrid=True,zeroline=False),
    legend=dict(bgcolor='rgba(0,0,0,0)',bordercolor='#d1dce8',font=dict(size=10)),
)
...
def gauge(val, target, direction, title, color):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'bordercolor': '#d1dce8',
            'steps': [
                {'range': [lo, ref], 'color': '#e8f0f7'},
                {'range': [ref, hi], 'color': '#ffffff'},
            ],
            'threshold': {
...
# ── LOAD ARTIFACTS ──────────────────────────────────────────────────────────
DASHBOARD_DIR = os.path.dirname(os.path.abspath(_file_))
...
with st.sidebar:
    ...
    st.markdown('<div style="font-size:0.75rem;color:#446183;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px">📅 DATE FILTER</div>', unsafe_allow_html=True)
    date_range = st.date_input('', value=(d_min, d_max), min_value=d_min, max_value=d_max, label_visibility='collapsed')
    st.markdown('---')
    st.markdown(f'<div style="font-size:0.72rem;color:#4a5568;text-align:center">Records: {len(sq_raw)} clean | Features: {sq_fe.shape[1]}<br>Models: RF · XGB · LGB · CAT<br>Last trained: current session</div>', unsafe_allow_html=True)
...
if '🏠' in page:
    ...
    for col,target,direction in zip([a1,a2,a3],TARGETS,['>=','<=','>=']):
        ...
        <div class="insight-box" style="text-align:center;padding:18px 14px">
          <div style="font-size:0.72rem;color:#446183;font-weight:600;letter-spacing:0.5px;margin-bottom:4px">
            {target} — 7-Day Moving Average
          </div>
          <div style="font-size:0.67rem;color:#4a5568;margin-bottom:10px">
            📅 {date_from} → {date_to}
          </div>
          <div style="font-size:1.9rem;font-weight:700;color:#1a3a5c;
                      font-family:'JetBrains Mono',monospace;line-height:1.1;margin-bottom:4px">
            {recent:.3f}
          </div>
          <div style="font-size:0.71rem;color:#6b7a8d;margin-bottom:10px">
            Target: {dir_sym}{tgt}
          </div>
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("""
    <div style="font-size:0.7rem;color:#446183;letter-spacing:1px;
                text-transform:uppercase;margin-bottom:14px;font-weight:600">
      🧠 FEATURE IMPORTANCE & SHAP ANALYSIS
      <span style="color:#4a5568;font-weight:400;margin-left:8px">
    ...
    for col,label,feat,val,tc,hint in [
        ...
        val_html = (f'<div style="font-size:1rem;font-weight:700;color:{tc};'
                    ...
                    f'{feat}</div>'
                    f'<div style="font-size:0.68rem;color:#6b7a8d">'
                    ...
                    f'<div style="font-size:0.65rem;color:#6b7a8d">{hint}</div>')
        col.markdown(
            f'<div style="background:#f8fafc;border:1px solid #d1dce8;'
            f'border-top:3px solid {tc};border-radius:10px;padding:14px 16px;">'
            f'<div style="font-size:0.65rem;color:#6b7a8d;text-transform:uppercase;'
    ...
    <div style="background:linear-gradient(135deg,#f8fafc,#ffffff);
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:0.76rem">
        <div style="background:#ffffff;border-radius:6px;padding:10px 12px;border-left:3px solid #58A6FF">
          <span style="color:#58A6FF;font-weight:600">TI:</span>
          <span style="color:#446183"> {ti_blurb}</span>
        </div>
        <div style="background:#ffffff;border-radius:6px;padding:10px 12px;border-left:3px solid #F85149">
          <span style="color:#F85149;font-weight:600">RDI:</span>
          <span style="color:#446183"> {rdi_blurb}</span>
        </div>
        <div style="background:#ffffff;border-radius:6px;padding:10px 12px;border-left:3px solid #3FB950">
          <span style="color:#3FB950;font-weight:600">RI:</span>
          <span style="color:#446183"> {ri_blurb}</span>
        </div>
        <div style="background:#ffffff;border-radius:6px;padding:10px 12px;border-left:3px solid #a855f7">
          <span style="color:#a855f7;font-weight:600">Common:</span>
          <span style="color:#446183"> {common_labels} appear in top-10 for all three targets —
    ...
    with tab_fi:
        st.markdown('<div style="font-size:0.72rem;color:#6b7a8d;margin-bottom:12px">'
        ...
        for col,target in zip([c1,c2,c3], TARGETS):
            with col:
                ...
                fig.add_vline(x=0, line=dict(color='#d1dce8', width=1))
                fig.update_layout(
                    paper_bgcolor='#f8fafc', plot_bgcolor='#ffffff',
                    font=dict(family='Inter', color='#94b4d4', size=10),
                    title=dict(
                        text=f'<b style="color:{tc}">{target}</b>  —  Feature Importance<br>'
                             f'<span style="font-size:10px;color:#4a5568">{bm[target]["best_algo"]} model</span>',
                        font=dict(size=13), x=0, xanchor='left'),
                    margin=dict(l=10, r=70, t=60, b=10),
                    height=80 + n_show * 34,
                    xaxis=dict(gridcolor='#d1dce8', zeroline=False, tickfont=dict(size=9),
                               title=dict(text='Importance Score', font=dict(size=9,color='#64748b'))),
                    yaxis=dict(gridcolor='#d1dce8', zeroline=False, tickfont=dict(size=9.5),
                               automargin=True),
                )
                st.plotly_chart(fig, use_container_width=True)
                ...
                st.markdown(
                    f'<div style="font-size:0.68rem;color:#6b7a8d;margin:-6px 0 8px;text-align:center">'
                ...
                fig.update_layout(
                    ...
                    title=dict(
                        text=f'<b style="color:{tc}">{target}</b>  —  SHAP Importance<br>'
                             f'<span style="font-size:10px;color:#4a5568">Mean |SHAP value| across all predictions</span>',
                        font=dict(size=13), x=0, xanchor='left'),
                    margin=dict(l=10, r=70, t=60, b=10),
                    height=80 + n_show * 34,
                    xaxis=dict(gridcolor='#d1dce8', zeroline=False, tickfont=dict(size=9),
    ...
    with tab_bee:
        target_sel = st.selectbox('Select Quality Target', TARGETS,
                                   ...
                                   key='bee_target')
        st.markdown(
            '<div style="font-size:0.72rem;color:#6b7a8d;margin-bottom:10px">'
        ...
        fig.update_layout(
            ...
            title=dict(
                text=f'<b style="color:{tc}">{target_sel}</b>  —  SHAP Beeswarm<br>'
                     f'<span style="font-size:10px;color:#4a5568">'
    ...
    with tab_dep:
        st.markdown('<div style="font-size:0.72rem;color:#6b7a8d;margin-bottom:10px">'
            ...
            fig.update_layout(
                ...
                xaxis=dict(gridcolor='#d1dce8', zeroline=False, tickfont=dict(size=9),
                           title=dict(text=f'{dep_sel}  (Feature Value)',
                                      font=dict(size=10, color='#64748b'))),
                yaxis=dict(gridcolor='#d1dce8', zeroline=False, tickfont=dict(size=9),
    ...
    with tab_wf:
        ...
        if n_rows == 0:
            ...
            fig = go.Figure(go.Waterfall(
                ...
                textposition='outside',
                textfont=dict(size=9, color='#94b4d4'),
                connector=dict(line=dict(color='#d1dce8', width=1)),
            ...
            fig.update_layout(
                ...
                yaxis=dict(gridcolor='#d1dce8', zeroline=False,
    ...
    with tab_met:
        for target in TARGETS:
            ...
            data   = sd[target]
            st.markdown(
                f'<div style="background:#f8fafc;border-left:4px solid {tc};'
                ...
                f'<span style="color:#6b7a8d;font-size:0.72rem;margin-left:10px">'
            ...
            for i,(feat,val) in enumerate(zip(data['top_feats'][:n_show],
                ...
                card = (
                    ...
                    f'<span style="font-size:0.68rem;background:#f8fafc;'
                    f'border:1px solid #d1dce8;border-radius:12px;'
                    f'padding:2px 8px;color:#446183">SHAP: {val:.4f}</span>'
                    f'</div>'
                    f'<div style="font-size:0.73rem;color:#446183;margin-bottom:4px">'
                    f'🔬 {mech}</div>'
